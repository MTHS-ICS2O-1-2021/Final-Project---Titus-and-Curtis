 ...
 */
 class GameSceneNormal extends Phaser.Scene {
  ...
  var carPosition = Math.floor(Math.random()* 12) + 1
    ...
    if (carPosition == 1) {
    const anNormalCar = this.physics.add.sprite(270, 100, 'normalCar').setScale(0.6)
    anNormalCar.body.velocity.y = carNormalSpeed
    this.normalCarGroup.add(anNormalCar)
    }
    if (carPosition == 2) {
      const anNormalCar = this.physics.add.sprite(540, 100, 'normalCar').setScale(0.6)
      anNormalCar.body.velocity.y = carNormalSpeed
      this.normalCarGroup.add(anNormalCar)
    }
    if (carPosition == 3) {
      const anNormalCar = this.physics.add.sprite(800, 100, 'normalCar').setScale(0.6)
     anNormalCar.body.velocity.y = carNormalSpeed
      this.normalCarGroup.add(anNormalCar)
    }
    // cars that spawn on the right lane
    if (carPosition == 4) {
      const anNormalCar = this.physics.add.sprite(980, 270, 'normalCarSide').setScale(0.6)
      anNormalCar.body.velocity.x = -carNormalSpeed
      this.normalCarGroup.add(anNormalCar)
    }
    if (carPosition == 5) {
      const anNormalCar = this.physics.add.sprite(980, 540, 'normalCarSide').setScale(0.6)
      anNormalCar.body.velocity.x = -carNormalSpeed
      this.normalCarGroup.add(anNormalCar)
    }
    if (carPosition == 6) {
      const anNormalCar = this.physics.add.sprite(980, 800, 'normalCarSide').setScale(0.6)
      anNormalCar.body.velocity.x = -carNormalSpeed
      this.normalCarGroup.add(anNormalCar)
    }
    // cars that spawn on the bottom lane
    if (carPosition == 7) {
      const anNormalCar = this.physics.add.sprite(800, 980, 'normalCar').setScale(0.6).setAngle(180)
      anNormalCar.body.velocity.y = -carNormalSpeed
      this.normalCarGroup.add(anNormalCar)
    }
    if (carPosition == 8) {
      const anNormalCar = this.physics.add.sprite(540, 980, 'normalCar').setScale(0.6).setAngle(180)
      anNormalCar.body.velocity.y = -carNormalSpeed
      this.normalCarGroup.add(anNormalCar)
    }
    if (carPosition == 9) {
      const anNormalCar = this.physics.add.sprite(270, 980, 'normalCar').setScale(0.6).setAngle(180)
      anNormalCar.body.velocity.y = -carNormalSpeed
      this.normalCarGroup.add(anNormalCar)
    }
     // cars that spawn on the left lane
    if (carPosition == 10) {
      const anNormalCar = this.physics.add.sprite(100, 800, 'normalCarSide').setScale(0.6).setAngle(180)
      anNormalCar.body.velocity.x = carNormalSpeed
      this.normalCarGroup.add(anNormalCar)
    }
    if (carPosition == 11) {
      const anNormalCar = this.physics.add.sprite(100, 540, 'normalCarSide').setScale(0.6).setAngle(180)
      anNormalCar.body.velocity.x = carNormalSpeed
      this.normalCarGroup.add(anNormalCar)
    }
    if (carPosition == 12) {
      const anNormalCar = this.physics.add.sprite(100, 270, 'normalCarSide').setScale(0.6).setAngle(180)
      anNormalCar.body.velocity.x = carNormalSpeed
      this.normalCarGroup.add(anNormalCar)
    }
  }
  ...
  create(data) {
    ...
    this.monkeyNormal = this.physics.add.sprite(1080 / 2, (1080 / 2), 'monkey').setScale(0.27)
    this.borderRight = this.physics.add.sprite(1060, 540 , 'borderVertical').setImmovable()
    this.borderBottom = this.physics.add.sprite(540, 1060 , 'borderHorizontal').setImmovable()
    this.borderLeft = this.physics.add.sprite(20, 540, 'borderVertical').setImmovable()
    this.borderTop = this.physics.add.sprite(540, 20, 'borderHorizontal').setImmovable()
   ...
   // create a group for the normalCars
    ...
    this.physics.add.collider(this.monkeyNormal, this.normalCarGroup, function (monkeyNormalCollide, normalCarCollide) {
    ...
    this.physics.add.collider(this.normalCarGroup, this.borderTop, function (normalCarCollide, borderCollide) {
    ...
    this.physics.add.collider(this.normalCarGroup, this.borderRight, function (normalCarCollide, borderCollide) {
    normalCarCollide.destroy()
    this.createNormalCar()
  }.bind(this))

          // Collisions between car and the bottom border
    this.physics.add.collider(this.normalCarGroup, this.borderBottom, function (normalCarCollide, borderCollide) {
    normalCarCollide.destroy()
    this.createNormalCar()
  }.bind(this))

          // Collisions between car and the left border
    this.physics.add.collider(this.normalCarGroup, this.borderLeft, function (normalCarCollide, borderCollide) {
    normalCarCollide.destroy()
    this.createNormalCar()
  }.bind(this))
  }
